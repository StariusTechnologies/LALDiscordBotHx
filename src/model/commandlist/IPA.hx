...
class IPA implements ICommandDefinition {
    public var paramsUsage = '(X-SAMPA)';
    ...
    public function new(context: CommunicationContext) {
        ...
        _context = context;
        description = LangCenter.instance.translate(serverId, 'model.commandlist.ipa.description');
    }
    ...
    public function process(args: Array<String>): Void {
        if (args.length > 0 && args[0].length > 0) {
            ...
            _context.rawSendToChannel(_context.getMessage().author + ' => ' + transcriptedString);
        } else {
            _context.sendToChannel('model.commandlist.ipa.process.fail', cast [_context.getMessage().author]);
        }
    }
